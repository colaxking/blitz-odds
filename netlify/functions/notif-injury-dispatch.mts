...
// Synchronous front door for notif-injury-dispatch-background, for the same
...
// said "accepted."
//
// A silent failure here is particularly hard to notice: no alerts and an
// empty review queue look exactly like a quiet week in the NFL.
//
// POST /.netlify/functions/notif-injury-dispatch
...
export default async (req: Request, _context: Context) => {
  ...
  try {
    const res = await fetch(`${origin}/.netlify/functions/notif-injury-dispatch-background`, {
      method: "POST",
  ...
  return jsonResponse(200, {
    ...
    note: "Configuration checks passed. Changes, alerts and queued review items are in the notif-injury-dispatch-background function log.",
  });
};

export const config: Config = {
  path: "/.netlify/functions/notif-injury-dispatch",
};
