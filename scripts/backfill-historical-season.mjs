...
.tab-btn {
  ...
  color: var(--text-dim); font-family: inherit; text-decoration: none; font-size: 0.92rem; font-weight: 700;
...
footer.app-footer p { margin: 0 0 8px; }
footer.app-footer p:last-child { margin-bottom: 0; }
/* Copyright + NFL non-affiliation, matching the live app's footer. The year
   is filled in by FOOTER_YEAR_SCRIPT at runtime; the hardcoded value in the
   span is the fallback a non-JS crawler sees. */
.footer-legal { border-top: 1px solid var(--card-border); padding-top: 10px; margin-top: 10px; font-size: 0.7rem; line-height: 1.5; }
...
// Fills the copyright year in the footer at page load, so a static archive
// page generated in one year doesn't still read that year forever. The span
// ships with a hardcoded fallback for crawlers that don't run JS.
const FOOTER_YEAR_SCRIPT = `(function () {
  try {
    var y = String(new Date().getFullYear());
    var nodes = document.querySelectorAll(".footer-year");
    for (var i = 0; i < nodes.length; i++) nodes[i].textContent = y;
  } catch (e) { /* leave the fallback year in place */ }
})();`;
 ...
 * The CSS block above is a copy of index.html's, adapted for anchors -
 * scripts/sync-archive-nav.mjs derives the same thing from index.html at
 * runtime and rewrites every existing archive page with it. If you change
 * the app's tab bar: update index.html, run that sync, and paste the block
 * it produces here so a newly generated season starts out matching too.
...
function archiveTab(href, label, icon) {
  return `    <a class="tab-btn" href="${href}"><span class="tab-icon has-img" aria-hidden="true"><img class="tab-icon-img" src="/branding/tab-icon-${icon}.png" alt="" width="40" height="40" decoding="async" /></span><span class="tab-label">${label}</span></a>`;
}
...
<div class="app">
  ...
  <footer class="app-footer">
    ...
    <p class="footer-legal">&copy; <span class="footer-year">2026</span> Blitz Odds. Not affiliated with or endorsed by the National Football League. Team names and marks are the property of their respective owners. <a href="/privacy/">Privacy</a></p>
  </footer>
</div>
<script src="/js/analytics.js"></script>
<script>${SETTINGS_WIDGET_SCRIPT}</script>
<script>${FOOTER_YEAR_SCRIPT}</script>
